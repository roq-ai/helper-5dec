...
import { getUpdates, deleteUpdateById } from 'apiSdk/updates';
import { UpdateInterface } from 'interfaces/update';

type ColumnType = ColumnDef<UpdateInterface, unknown>;

interface UpdateListPageProps {
...
export function UpdateListPage(props: UpdateListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<UpdateInterface>>(
    () => `/updates?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: UpdateInterface) => {
    if (hasAccess('update', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/updates/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/updates/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('update', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/updates/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('update', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="update" />
        </Flex>

        {hasAccess('update', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/updates/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
